  ...
  class_methods do
    def config_for(chat)
      ...
      if chat.user.ui_layout_intro?
        ...
        {
          instructions: custom || default_instructions(preferred_currency, preferred_date_format, user: chat.user),
          functions: default_functions(chat.user)
        }
      end
    end

    def default_instructions_for(family)
      default_instructions(Money::Currency.new(family.currency), family.date_format)
    end
